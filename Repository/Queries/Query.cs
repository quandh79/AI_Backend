...
namespace Repository.Queries
{
    public class Query<T>
    {
        ...
        private BCC01_DbContextSql dbContext { get; set; }
        public int page { get; set; } = 0;
        public int limit { get; set; } = 0;

        public DynamicParameters parameters { get; set; } = new DynamicParameters();

        public IStoreProcedureExcute _storeProcedure;
        public IStoreProcedureExcute StoreProcedure
        {
            get
            {
                return _storeProcedure ?? (_storeProcedure = new StoreProcedureExcute());
            }
        }
        public Query()
        {
        ...
        public Query(BCC01_DbContextSql dbContext)
        {
            this.dbContext = dbContext;
        }

        public Query(PagingParam param, BCC01_DbContextSql dbContext)
        {
            this.dbContext = dbContext;
        ...
        public Query<T> Where(SearchParam param)
        {
            ...
            var nameParame = RemoveSpecialCharacters(param.name_field);  //param.name_field?.Replace("[b].", ""); 
            PropertyInfo property = typeof(T).GetProperty(nameParame);
            if (property == null)
            ...
            parameters.Add($"@{nameParame}_val", param.value_search);

            if (param.upper_bound == null)
                conditions += $"{param.name_field} = @{nameParame}_val ";
            else
            {
                parameters.Add($"@{nameParame}_upp", param.upper_bound);
                conditions += $"({param.name_field} > @{nameParame}_val AND {param.name_field} < @{nameParame}_upp ) ";
            }
        ...
        public Query<T> WhereLike(SearchParam param)
        {
            ...
            var nameParame = RemoveSpecialCharacters(param.name_field); //param.name_field.Replace("[b].", "");
            parameters.Add($"@{nameParame}_val", valueSearch);

            if (isUnicode)
                conditions += $"CONVERT(VARCHAR(max), {param.name_field}, 103) LIKE '%' + @{nameParame}_val + '%' ";
            // else convert datetime then search
            else if (property.PropertyType == typeof(DateTime) || property.PropertyType == typeof(DateTime?))
                conditions += $"[dbo].[ufn_convertDatetime]({param.name_field}) LIKE '%' + @{nameParame}_val + '%' ";
            // else convert value from db then search unaccented
            else
                conditions += $"CONVERT(VARCHAR(max), dbo.ufn_removeMark({param.name_field}), 103) LIKE '%' + @{nameParame}_val + '%' ";
        ...
        public async Task<List<T>> ToListAsync()
        {
            ...
            var query = (await StoreProcedure.ExecuteReturnListBySql<T>(queryString, parameters, 1)).ToList();
        ...
        public async Task<int> CountAsync()
        {
            ...
            return (await StoreProcedure.ExecuteReturnListBySql<T>(queryString, parameters,1)).Count();
        }
